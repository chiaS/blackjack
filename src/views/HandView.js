...
window.HandView = (function(_super) {
  ...
  HandView.prototype.template = _.template('<h2><% if(isDealer){ %>Dealer<% }else{ %>Your<% } %> Score: <span class="score"></span></h2>');

  HandView.prototype.initialize = function() {
    this.collection.on('reveal add remove change', (function(_this) {
      return function() {
        return _this.render();
      };
    })(this));
  ...
  HandView.prototype.render = function() {
    ...
    return this.$('.score').text(this.collection.finalScore());
  };
